...
(defun -nomis/wc/frame->frame-info (frame)
  (let* ((display-width  (display-pixel-width frame))
    ...
    (puthash :display-width  display-width  info)
    (puthash :display-height display-height info)
...
(defun -nomis/wc/apply-frame-info-to-frame (frame info)
  (let* ((display-width        (display-pixel-width frame))
         (display-height       (display-pixel-height frame))
         (saved-display-width  (gethash :display-width  info))
         (saved-display-height (gethash :display-height info))
         (saved-left           (gethash :left   info))
         (saved-top            (gethash :top    info))
         (saved-width          (gethash :width  info))
         (saved-height         (gethash :height info))
         (state                (gethash :state  info))
         (x-multiplier (if saved-display-width
                           (/ (float display-width) saved-display-width)
                         1))
         (y-multiplier (if saved-display-height
                           (/ (float display-height) saved-display-height)
                         1))
         (left         (floor (* x-multiplier saved-left)))
         (top          (floor (* y-multiplier saved-top)))
         (width        (floor (* x-multiplier saved-width)))
         (height       (floor (* y-multiplier saved-height))))
