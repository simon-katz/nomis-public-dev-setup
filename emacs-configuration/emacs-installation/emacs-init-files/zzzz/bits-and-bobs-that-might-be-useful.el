...
(defun zzzz-nomis/dirtree/full-filename->list-from-file-system-root (full-filename)
  (assert (eq ?/ (string-to-char full-filename)))
...
(progn ; tests
  (let ((strings '("/aa/bbb/cccc/"
                   "/aa/bbb/cccc")))
    (assert (equal (-map #'zzzz-nomis/dirtree/remove-trailing-slash
                         strings)
                   '("/aa/bbb/cccc"
                     "/aa/bbb/cccc")))
    (assert (equal (-map #'zzzz-nomis/dirtree/full-filename->list-from-file-system-root
                         strings)
                   '(("aa" "bbb" "cccc")
                     ("aa" "bbb" "cccc"))))))
