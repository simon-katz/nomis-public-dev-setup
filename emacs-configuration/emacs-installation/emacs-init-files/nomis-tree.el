            ...
            (unless *-nomis/tree/inhibit-set-level-etc-message?*
              (funcall (if error?
                       ...
                       (if -nomis/tree/show-bodies?
                         ...
                         " (not showing bodies)")
                       (if error?
                           (if (< requested-value maximum-value)
...
When in a body, \"current heading\" means the current body's parent heading."
  ...
  (-nomis/tree/command
    ...
    (if n-or-nil
      ...
      (let* ((start-level (-nomis/tree/start-level-for-incremental-contract/point)))
        (-nomis/tree/set-level-etc :point (1- start-level) :collapse)))))
...
If N-OR-NIL is provided, set the number of child levels to N-OR-NIL."
  ...
  (-nomis/tree/command
    ...
    (if n-or-nil
      ...
      (let* ((start-level (-nomis/tree/start-level-for-incremental-contract/root)))
        (-nomis/tree/set-level-etc :root (1- start-level) :collapse)))))
...
If N-OR-NIL is provided, set the number of child levels to N-OR-NIL."
  ...
  (-nomis/tree/command
    ...
    (if n-or-nil
      ...
      (let* ((start-level (-nomis/tree/start-level-for-incremental-contract/buffer)))
        (-nomis/tree/set-level-etc :all-roots (1- start-level) :collapse)))))
