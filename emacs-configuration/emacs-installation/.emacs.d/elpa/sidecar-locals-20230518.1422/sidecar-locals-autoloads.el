...
This is a minor mode.  If called interactively, toggle the `Sidecar-Locals mode'
...
is nil, omitted, or is a positive number.  Disable the mode if ARG is
a negative number.

To check whether the minor mode is enabled in the current buffer, evaluate
`(default-value \\='sidecar-locals-mode)'.
